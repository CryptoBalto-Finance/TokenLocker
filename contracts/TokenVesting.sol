...
contract TokenVesting is Ownable, ReentrancyGuard {
  ...
  constructor (IBaltosafeAdmin _safeAdmins) {
    BALTO_ADMINS = _safeAdmins;
    FEES.tokenFee = 35;
    FEES.feeAddress = payable();
  ...
  // Callable by BALTO_ADMINS or whitelisted contracts (such as presale contracts)
  function editZeroFeeWhitelist (address _token, bool _add) external {
    require(BALTO_ADMINS.userIsAdmin(msg.sender) || TOKEN_WHITELISTERS.contains(msg.sender), 'ADMIN');
    if (_add) {
      ZERO_FEE_WHITELIST.add(_token);
    } else {
